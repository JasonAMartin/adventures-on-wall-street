...
namespace EnhancedScrollerDemos.NestedScrollers
{
    ...
    public class Controller : MonoBehaviour, IEnhancedScrollerDelegate
    {
        ...
        /// </summary>
        private List<List<DetailData>> _data;
        ...
        private void LoadData()
        {
            ...
            _data = new List<List<DetailData>>();
            for (var i = 0; i < 1000; i++)
            {
                var detailList = new List<DetailData>();

                _data.Add(detailList);

                for (var j = 0; j < 20; j++)
                detailList.Add(new DetailData() { someText = i.ToString() + "," + j.ToString() });
            }
